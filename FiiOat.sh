...
# Cache installed package list for quicker lookups firmwares where system apps shifted
# Credit to kuiporro (GitHub)
INSTALLED_PACKAGES="$(pm list packages 2>/dev/null)"
log_info "Installed packages detected for debloat and control: $(echo "$INSTALLED_PACKAGES" | wc -l)"
...
# Function to check if a package exists (for packages listed in code)
# Credit to kuiporro (GitHub)
package_exists() {
    echo "$INSTALLED_PACKAGES" | grep -q "^package:$1$"
}

# Function to disable a package if it exists (for packages listed in code)
# Credit to kuiporro (GitHub)
disable_pkg() {
    local pkg="$1"
...
# Function to force stop a package if it exists (for packages listed in code)
# Credit to kuiporro (GitHub)
force_stop_pkg() {
    local pkg="$1"
    if package_exists "$pkg"; then
...
# Function to set appops background permission if package exists (for packages listed in code)
# Credit to kuiporro (GitHub)
...
# Function to whitelist a package if it exists (for packages listed in code)
# Credit to kuiporro (GitHub)
whitelist_pkg() {
    local pkg="$1"
    if package_exists "$pkg"; then
...
log_info "Starting FiiOat $RELEASE_VERSION"
...
# log each app permission set to backgopund
...
# Set Music Apps to use unrestricted mode when DAP is on Battery power
# log all whitelisted apps
log_info "Whitelisting music applications for unrestricted battery use..."
...
su -lp 2000 -c "cmd notification post -S bigtext -t 'FiiO Android Tweaker' 'Tag' 'Born for Music! FiiO Android Tweaker $RELEASE_VERSION successfully installed.'" >/dev/null 2>&1