# Installation and Testing Guide - FiiOat_v17_r46

## ⚠️ IMPORTANT: Stable Version

This version (v17_r46) has been **tested and is stable**. Ready for production use.

## Prerequisites

- FiiO JM21 with firmware 1.0.8 or newer
- Fiio M21 with firmware 1.0.5 or newer
...
2. Download the latest release: `FiiOat_v17_r46.zip`
...
2. Run the build script:
   ...
   ./build_module.sh
   ```
3. The script will automatically create the ZIP with the correct name: `FiiOat_v17_r46.zip`
...
2. Use `zip`, `7z` or `python3`:
   ...
   **With zip:**
   ```bash
   zip -r FiiOat_v17_r46.zip META-INF/ FiiOat.sh service.sh module.prop customize.sh
   ```

   **With 7z:**
   ```bash
   7z a -tzip FiiOat_v17_r46.zip META-INF/ FiiOat.sh service.sh module.prop customize.sh
   ```
   ...
   **With Python3:**
   ...
   with zipfile.ZipFile('FiiOat_v17_r46.zip', 'w') as zipf:
...
2. Transfer the `FiiOat_v17_r46.zip` file to the device's internal storage
3. Or transfer via ADB:
   ```bash
   adb push FiiOat_v17_r46.zip /sdcard/Download/
   ```
...
# Install module
magisk --install-module /sdcard/Download/FiiOat_v17_r46.zip
...
4. You should see version `v17_r46`
...
2. **Verify Firmware Detection**:
   ...
   adb shell
   su
   grep "1.1.1" /data/adb/modules/fiioat/info.log
...
- [ ] Apps work normally
- [ ] Firmware detection works (if applicable)
