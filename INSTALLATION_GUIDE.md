# Installation and Testing Guide - FiiOat_v17_r45

## ⚠️ IMPORTANT: Stable Version

This version (v17_r45) has been **tested and is stable**. Ready for production use.
...
2. Download the latest release: `FiiOat_v17_r45.zip`
...
2. Run the build script:
   ...
   ./build_module.sh
   ```
3. The script will automatically create the ZIP with the correct name: `FiiOat_v17_r45.zip`
...
2. Use `zip`, `7z` or `python3`:
   ...
   **With zip:**
   ```bash
   zip -r FiiOat_v17_r45.zip META-INF/ FiiOat.sh service.sh module.prop customize.sh
   ```

   **With 7z:**
   ```bash
   7z a -tzip FiiOat_v17_r45.zip META-INF/ FiiOat.sh service.sh module.prop customize.sh
   ```
   ...
   **With Python3:**
   ...
   with zipfile.ZipFile('FiiOat_v17_r45.zip', 'w') as zipf:
...
2. Transfer the `FiiOat_v17_r45.zip` file to the device's internal storage
3. Or transfer via ADB:
   ```bash
   adb push FiiOat_v17_r45.zip /sdcard/Download/
   ```
...
# Install module
magisk --install-module /sdcard/Download/FiiOat_v17_r45.zip
...
4. You should see version `v17_r45`
