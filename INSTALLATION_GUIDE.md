# Installation and Testing Guide - FiiOat_v17_r49

## ⚠️ IMPORTANT: Stable Version

This version (v17_r49) has been **tested and is stable**. Ready for production use.
...
2. Download the latest release: `FiiOat_v17_r49.zip`
...
2. Use `zip`, `7z` or `python3`:
   ...
   **With zip:**
   ```bash
   zip -r FiiOat_v17_r49.zip META-INF/ FiiOat.sh service.sh module.prop customize.sh
   ```

   **With 7z:**
   ```bash
   7z a -tzip FiiOat_v17_r49.zip META-INF/ FiiOat.sh service.sh module.prop customize.sh
   ```
   ...
   **With Python3:**
   ...
   with zipfile.ZipFile('FiiOat_v17_r49.zip', 'w') as zipf:
       for root, dirs, files in os.walk('META-INF'):
           for file in files:
               zipf.write(os.path.join(root, file))
       for file in ['FiiOat.sh', 'service.sh', 'module.prop', 'customize.sh', 'action.sh']:
...
2. Transfer the `FiiOat_v17_r49.zip` file to the device's internal storage
3. Or transfer via ADB:
   ```bash
   adb push FiiOat_v17_r49.zip /sdcard/Download/
   ```
...
# Install module
magisk --install-module /sdcard/Download/FiiOat_v17_r49.zip
...
4. You should see version `v17_r49`
